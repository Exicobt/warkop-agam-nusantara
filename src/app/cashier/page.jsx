...
const KasirDashboard = () => {
  ...
  const debugOrders = async () => {
    try {
      const res = await fetch("/api/orders?status=all");
      const data = await res.json();
      console.log("Debug order data:", data[0].total); // Cek apakah ada field total
    } catch (err) {
  ...
  const handlePrintBill = (order) => {
    // Simulasi print struk
    showNotification("info", `Struk pesanan #${order.id} sedang diprint...`);
  };
  ...
  orders.forEach(o => {
    ...
    if (o.status === "finish") {
      ...
      if (orderDate.getTime() === today.getTime()) {
        // GUNAKAN item.total BUKAN item.harga * item.qty
    ...
    <div className="w-full min-h-screen font-sans mt-10">
      ...
      <div className="flex pt-6 px-4 sm:px-6 pb-6 flex-col max-w-7xl mx-auto">
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3 mb-6">
          ...
          <div className="bg-white p-3 rounded-xl shadow-sm border border-indigo-100 flex flex-col">
            <div className="flex items-center justify-between mb-1">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-3 text-xs text-gray-500">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <span>Meja {order.customers?.table?.table_number || "Takeaway"}</span>
                          </div>
                          <div className="flex items-center gap-1">
